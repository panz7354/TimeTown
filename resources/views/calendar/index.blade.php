...
.form-group label{font-size:11px;color:#8B6914;letter-spacing:0.05em;}
.form-group input,
.form-group select{
  background:#F5EDD6;
  border:1.5px solid #D4C089;
  ...
  font-size:13px;color:#3D2B1F;
  outline:none;
}
.form-group input:focus,
.form-group select:focus{border-color:#C9A84C;}
.form-group.grow{flex:1;min-width:160px;}
.submit-btn{
  background:#C9A84C;color:#3D2B1F;
  border:none;border-radius:8px;
  padding:9px 22px;font-family:'Noto Serif TC',serif;
  font-size:13px;font-weight:700;cursor:pointer;
  transition:background 0.15s;white-space:nowrap;
  align-self:flex-end;
}
.submit-btn:hover{background:#B89040;}
...
.task-card:hover{border-color:#C9A84C;}
...
.task-info{flex:1;}
.task-title{font-size:14px;color:#3D2B1F;margin-bottom:3px;}
.task-title.done{text-decoration:line-through;color:#8B7A6A;}
.task-meta{font-size:11px;color:#9A7230;display:flex;gap:8px;flex-wrap:wrap;}
.task-type-badge{
...
.complete-btn{
  background:none;border:1.5px solid #C9A84C;
  border-radius:8px;padding:5px 14px;
  font-family:'Noto Serif TC',serif;font-size:12px;
  color:#8B6914;cursor:pointer;transition:all 0.15s;
  white-space:nowrap;
}
.complete-btn:hover{background:#C9A84C;color:#3D2B1F;}
.done-mark{
  font-size:12px;color:#6A9E4E;font-weight:700;
  padding:5px 10px;
}
    ...
    <div class="task-list">
      @foreach($completed as $task)
        ...
        <div class="task-card completed">
          ...
          <span class="task-type-badge"
                style="background:{{ $c['bg'] }};color:{{ $c['text'] }};opacity:0.7">
            {{ $task->type }}
          </span>
