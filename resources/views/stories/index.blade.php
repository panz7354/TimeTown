...
.main{max-width:760px;margin:0 auto;padding:28px 20px 100px;}

/* Flash */
.flash{padding:10px 16px;border-radius:10px;font-size:13px;margin-bottom:18px;}
.flash-success{background:#EAF3DE;color:#3B6D11;border:1px solid #C0DD97;}
.flash-info{background:#FAEEDA;color:#854F0B;border:1px solid #FAC775;}
.flash-error{background:#FAEDED;color:#A32D2D;border:1px solid #F5A5A5;}
...
.week-selector label{font-size:12px;color:#8B6914;font-weight:700;}
.week-selector select{
    background:#F5EDD6;border:1.5px solid #D4C089;border-radius:8px;
    padding:6px 12px;font-family:'Noto Serif TC',serif;font-size:13px;color:#3D2B1F;outline:none;
}

/* 生成按鈕區 */
.generate-box{
    background:#FDF7EC;border:1.5px dashed #C9A84C;border-radius:14px;
    padding:28px;text-align:center;margin-bottom:24px;
}
.generate-hint{font-size:13px;color:#8B6914;margin-bottom:16px;line-height:1.7;}
.generate-btn{
    background:#C9A84C;color:#3D2B1F;border:none;border-radius:10px;
    padding:11px 28px;font-family:'Noto Serif TC',serif;font-size:14px;font-weight:700;
    cursor:pointer;transition:background 0.15s;
}
.generate-btn:hover{background:#B89040;}
.generate-btn:disabled{background:#D4C089;cursor:not-allowed;}

/* 故事卡片 */
.story-card{
    background:#FDF7EC;border:1.5px solid #D4C089;border-radius:18px;
    padding:32px 36px;margin-bottom:20px;
    box-shadow:0 2px 12px rgba(61,43,31,0.07);
}
.story-week-label{
    font-size:11px;color:#9A7230;letter-spacing:0.1em;
    margin-bottom:6px;
}
.story-divider{
    height:1px;background:linear-gradient(90deg,transparent,#D4C089,transparent);
    margin:18px 0;
}
.story-text{
    font-size:15px;line-height:2.1;color:#3D2B1F;
...
.story-generated{font-size:11px;color:#B8A880;}
.story-tasks-toggle{
    font-size:11px;color:#8B6914;cursor:pointer;
    border:1px solid #D4C089;border-radius:99px;padding:3px 12px;
    background:none;font-family:'Noto Serif TC',serif;
}
.story-tasks-toggle:hover{background:#F5EDD6;}

/* 任務摘要展開 */
.task-summary-list{
    margin-top:14px;padding:14px 16px;
    background:rgba(245,237,214,0.6);border-radius:10px;
    display:none;
}
.task-summary-list.show{display:block;}
.tsm-item{
    font-size:12px;color:#6B4C35;padding:3px 0;
    display:flex;align-items:center;gap:8px;
}
.tsm-done{color:#6A9E4E;}
.tsm-pending{color:#C9A84C;}

/* 空狀態 */
.empty-state{text-align:center;padding:60px 20px;color:#9A7230;}
...
.loading-text{color:#F5EDD6;font-size:15px;letter-spacing:0.08em;}
.spinner{
    width:40px;height:40px;border:3px solid rgba(245,237,214,0.3);
...
/* 導覽 */
.nav{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);display:flex;gap:10px;z-index:50;}
.nav-btn{
    background:rgba(61,43,31,0.85);color:#F5EDD6;border:1px solid rgba(201,168,76,0.5);
    padding:8px 20px;border-radius:99px;font-family:'Noto Serif TC',serif;font-size:13px;
    ...
    <div class="week-selector">
        ...
        ">
            @forelse($availableWeeks as $aw)
                <option value="{{ $aw->year }}-{{ $aw->week }}"
                    {{ $aw->year == $year && $aw->week == $week ? 'selected' : '' }}>
    ...
    @elseif($story && $story->story_text)
        ...
        <div class="story-card">
            ...
            <div class="story-footer">
                ...
                <button class="story-tasks-toggle" onclick="toggleTasks(this)">
                    查看本週任務摘要
                </button>
            </div>
